...
def define_net(input_nc, ncf, ninput_edges, nclasses, opt, gpu_ids, arch, init_type, init_gain, batch_size):
    ...
    if arch == 'mconvnet':
        net = MeshConvNet(input_nc, ncf, nclasses,
                              ninput_edges, opt.pool_res, 
                              opt.fc_n, batch_size, opt.resblocks)
    elif arch == 'mmlpnet':
        ...
        net = MeshPoolNet(input_nc, ncf, nclasses, ninput_edges, opt.pool_res, opt.fc_n, opt.norm, batch_size,
                          opt.resblocks)
    else:
