...
class PyrmCNet(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch=None):
        ...
        x0, edge_index0, batch0 = self.encoder0(x, edge_index, batch)   # 750 x 5 -> 600 x 64
        embeddings['encoder0'] = x0
        ...
        # x3 = self.linear(torch.cat([x0, x1, x2], dim=0))  # (600 + 360 + 144) x 64 ->
        x3 = torch.cat([x0, x1, x2], dim=0)
        batch3 = torch.cat([batch0, batch1, batch2], dim=0)
        xg = global_mean_pool(x3, batch3)
        embeddings['gb_pool'] = xg
